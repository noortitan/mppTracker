...
def set_fan_speed(ser, fan1_speed, fan2_speed):
    set_fan1_speed(ser, fan1_speed)
    set_fan2_speed(ser, fan2_speed)
    print(f"Fan speeds set: Fan1={fan1_speed}, Fan2={fan2_speed}")

def grid_search_fan_speeds(ser, BASE_DIR, save_as_channels=True):
    sensor_ids, filename = initialize_T_sensors(ser, BASE_DIR, save_as_channels)
    fan_speeds = [20, 40, 60, 80, 100]  # Range of speeds
    ...
    for fan1_speed in fan_speeds:
        for fan2_speed in fan_speeds:
            set_fan_speed(ser, fan1_speed, fan2_speed)
            start_time = time.time()
            
            while time.time() - start_time < 600:  # 20 minutes
                timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
                ...
                row = f"{timestamp},{elapsed_minutes},{fan1_speed},{fan2_speed}," + ",".join(temperatures)
                with open(filename, "a") as file:
                    file.write(row + "\n")
...
if __name__ == '__main__':
    ...
    port = "COM7"  
    baud_rate = 9600  
    BASE_DIR = "U:/20241105_microMPPT/data/20250207_T_fan_test/"
    try:
